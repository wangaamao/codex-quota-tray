...
它可以始终显示在 Windows 任务栏及普通窗口上方，根据文字自动调整大小，每分钟刷新一次，并且只允许运行一个实例。
...
运行编译好的 EXE 不需要安装 Python、Node.js、Visual Studio，不需要填写 API Key，也不需要创建配置文件。
...
程序每 750 毫秒重新确认一次置顶状态。使用管理员权限的应用和少数独占全屏程序仍可能遮挡普通权限窗口。通常不建议为了悬浮显示而长期使用管理员身份运行本工具。

### 多显示器或显示缩放

悬浮标签最初显示在主屏幕任务栏附近，之后可以拖到其他屏幕。第三方任务栏、竖向任务栏或特殊 DPI 设置可能影响初始位置，但不会影响额度读取。
