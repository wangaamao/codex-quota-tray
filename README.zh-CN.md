...
<img width="239" height="44" alt="image" src="https://github.com/user-attachments/assets/40efd1f7-660c-4812-9d7e-3af0c995e306" />


它可以始终显示在 Windows 任务栏及普通窗口上方，根据文字自动调整大小，默认每 5 分钟刷新一次，并且只允许运行一个实例。

> [!IMPORTANT]
> Codex Quota Tray 绝不会向任何地方发送 Codex 账户数据、额度、提示词、令牌或本地文件。额度读取仍然只通过本机 Codex 进程完成。打开右键菜单时，程序会匿名请求一次 GitHub 公开 Releases API（结果缓存 6 小时），仅用于显示最新公开版本号。Codex 自身可能像正常使用时一样连接 OpenAI。
...
- 可以拖动到任意位置。
- 右键菜单提供立即刷新、刷新间隔设置、打开 Codex、打开 GitHub、版本信息和退出功能。
...
本仓库没有独立后端、遥测服务、分析 SDK、自动更新器、数据库或隐藏的常驻后台服务。打开右键菜单时只会从 `api.github.com` 请求最新公开发布信息，结果在内存中缓存 6 小时。
...
- 通过网络发送 Codex 账户数据、额度、提示词、令牌、设备标识或本地文件；
...
打开右键菜单时，余量条会向 GitHub 公开的最新发布接口发起 HTTPS GET 请求。请求只包含通用的 `CodexQuotaTray/<版本>` User-Agent，不包含 Codex 账户或额度数据，结果在内存中缓存 6 小时。选择 **Open GitHub** 时，程序会让 Windows 使用默认浏览器打开公开仓库。

另外，本机 **Codex** 进程在检查登录状态或获取当前限额时，可能连接 OpenAI。这与用户正常运行 Codex 时的网络行为相同。
...
- 拖动悬浮标签：改变位置。
- 双击悬浮标签：立即刷新数据。
- 右键悬浮标签：刷新、选择刷新间隔、打开 Codex、打开 GitHub、查看当前/最新版本或退出。
...
Codex Quota Tray 1.3.3 基于 Codex CLI `0.142.0` 开发和验证。
